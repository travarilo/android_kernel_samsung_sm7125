...
static const char * const reset_gpio_label[2] = {
	"TAS256X_RESET", "TAS256X_RESET2"
};

static const char * const irq_gpio_label[2] = {
				  ...
				  struct snd_ctl_elem_value *ucontrol)
{
	...
	if (strnstr(ucontrol->id.name, "Version", MAX_STRING)) {
		...
		else if (strnstr(ucontrol->id.name, "RECEIVER ENABLE",
			MAX_STRING))
			ucontrol->value.integer.value[0] =
				p_tas256x->devs[0]->receiver_enable;
		else
		...
		if (strnstr(ucontrol->id.name, "LIMITER SWITCH",
			...
			MAX_STRING))
			ucontrol->value.integer.value[0] =
				p_tas256x->devs[1]->receiver_enable;
		else
			dev_err(plat_data->dev, "Invalid control %s\n",
				ucontrol->id.name);
	} else {
				  ...
				  struct snd_ctl_elem_value *ucontrol)
{
	...
	/* 24-09-20 Added Braces to Support Multplie lines in if else loop */
	if (strnstr(ucontrol->id.name, "LEFT", MAX_STRING)) {
		if (strnstr(ucontrol->id.name, "LIMITER SWITCH", MAX_STRING)) {
			ret = tas256x_update_limiter_enable(p_tas256x,
				ucontrol->value.integer.value[0],
				channel_left);
		} else if (strnstr(ucontrol->id.name, "BOP ENABLE", MAX_STRING)) {
			ret = tas256x_update_bop_enable(p_tas256x,
				ucontrol->value.integer.value[0],
				channel_left);
		} else if (strnstr(ucontrol->id.name, "BOP MUTE", MAX_STRING)) {
			ret = tas256x_update_bop_mute(p_tas256x,
				ucontrol->value.integer.value[0],
				channel_left);
		} else if (strnstr(ucontrol->id.name, "BROWNOUT SHUTDOWN", MAX_STRING)) {
			ret = tas256x_update_bop_shutdown_enable(p_tas256x,
				ucontrol->value.integer.value[0],
				channel_left);
		} else if (strnstr(ucontrol->id.name, "VBAT LPF", MAX_STRING)) {
			ret = tas256x_update_vbat_lpf(p_tas256x,
				ucontrol->value.integer.value[0],
				channel_left);
		} else if (strnstr(ucontrol->id.name, "RECEIVER ENABLE", MAX_STRING)) {
			ret = tas256x_enable_receiver_mode(p_tas256x,
				ucontrol->value.integer.value[0],
				channel_left);
			/* Added to Support to Bypass Algorithm get/Set During RCV Mode */
...
static const struct snd_kcontrol_new tas256x_left_controls[] = {
	...
	SOC_SINGLE_EXT("TAS256X CLASSH TIMER LEFT", CLASSH_TIMER, 1, 22, 0,
		tas256x_get, tas256x_put),
	SOC_ENUM_EXT("TAS256X RECEIVER ENABLE LEFT", tas256x_rx_switch_enum[0],
		tas256x_enum_get, tas256x_enum_put),
};
...
static const struct snd_kcontrol_new tas256x_right_controls[] = {
	...
	SOC_SINGLE_EXT("TAS256X CLASSH TIMER RIGHT", CLASSH_TIMER, 2, 22, 0,
		tas256x_get, tas256x_put),
	SOC_ENUM_EXT("TAS256X RECEIVER ENABLE RIGHT", tas256x_rx_switch_enum[0],
		tas256x_enum_get, tas256x_enum_put),
};
			...
			const struct i2c_device_id *id)
{
	...
	plat_data = devm_kzalloc(&p_client->dev,
